...
async def _live_redis_buses() -> AsyncIterator[tuple[RedisBus, RedisBus]]:
...
def test_orchestrator_worker_task_exchange() -> None:
    ...
    async def _run() -> None:
        async with _live_redis_buses() as (orchestrator_bus, worker_bus):
...
def test_shared_state_coordination() -> None:
    ...
    async def _run() -> None:
        async with _live_redis_buses() as (bus1, bus2):
...
def test_pubsub_event_communication() -> None:
    ...
    async def _run() -> None:
        async with _live_redis_buses() as (publisher_bus, subscriber_bus):
            ...
            subscribers_count = await publisher_bus.publish(
                channel,
                event_message,
            )
    ...
    async def _run() -> None:
        async with _live_redis_buses() as (orchestrator_bus, worker_bus):
            ...
            async def _orchestrator_listener() -> None:
                subscriber_gen = orchestrator_bus.subscribe(result_channel)
                async for result_envelope in subscriber_gen:
